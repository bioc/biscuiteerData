#' @title Supplementary Datasets for Biscuiteer
#'
#' @description A handful of datasets to be used in biscuiteer
#'
#' @author Timothy J Triche Jr \email{Tim.Triche@vai.org},
#' @author Wanding Zhou \email{Wanding.Zhou@vai.org},
#' @author Ben Johnson \email{Ben.Johnson@vai.org},
#' @author Jacob Morrison \email{Jacob.Morrison@vai.org},
#' @author Lyong Heo \email{Lyong.Heo@vai.org}
#'
#' @seealso Useful links:
#'          \itemize{
#'            \item \url{https://github.com/trichelab/biscuiteer}
#'            \item Report bugs at
#'                  \url{https://github.com/trichelab/biscuiteer/issues}
#'          }
#'
#' @aliases biscuiteerData
#'
#' @examples
#'
#'   Zhou_solo_WCGW_inCommonPMDs.hg19 <-
#'       biscuiteerDataGet("Zhou_solo_WCGW_inCommonPMDs.hg19.rda")
#'   PMDs.hg19 <- biscuiteerDataGet("PMDs.hg19.rda")
#'
#' @keywords DataImport MethylSeq DNAMethylation EpigeneticsWorkflow
#' @keywords Biscuit ExperimentData
#'
"_PACKAGE"
